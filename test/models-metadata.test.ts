...
let capturedConfig: { models?: unknown[] } | undefined;
...
// 静态 models：/models 在扩展加载期抓取一次并映射为 toOmnirouteModel 结果
async function loadModels(data: Entry[]): Promise<Array<Record<string, unknown>>> {
  mock.method(globalThis, "fetch", async () => okResponse(data));
  await entry(mockPi());
  return (capturedConfig!.models ?? []) as Array<Record<string, unknown>>;
}
...
test("contextWindow: max_input_tokens 缺失时用 context_length", async () => {
  const [m] = await loadModels([{ id: "m2", context_length: 131072 }]);
  assert.equal(m.contextWindow, 131072);
});

test("contextWindow: 两者均缺失时回退 128000", async () => {
  const [m] = await loadModels([{ id: "m3" }]);
...
test("input: capabilities.vision 为 true 时声明图片输入", async () => {
  const [m] = await loadModels([{ id: "m1", capabilities: { vision: true } }]);
...
test("input: input_modalities 非数组时按无视觉证据处理", async () => {
  const [m] = await loadModels([{ id: "m9", input_modalities: "text,image" }]);
  assert.deepEqual(m.input, ["text"]);
});

test("contextWindow: 字符串 max_input_tokens 按缺失处理", async () => {
  const [m] = await loadModels([{ id: "m5", max_input_tokens: "100" }]);
  assert.equal(m.contextWindow, 128000);
});
