...
- **Tool**
  ...
  - **成本遥测** —— 将 OmniRoute `X-OmniRoute-*` 遥测的真实美元成本与网关速率（tok/s、TTFT）写入 Pi 的用量/成本统计，完整遥测附加到每条消息的 `diagnostics`。
...
OmniRoute 在每个流式响应体的尾部以 SSE 注释行（`: x-omniroute-*`）下发单次请求的遥测数据（成本、token、速率、路由）。扩展拦截字节流、解析这些行，并接入 Pi 的计费体系：
...
- **网关速率指标** —— 解析 `X-OmniRoute-Tokens-Per-Second`（生成速率）与 `X-OmniRoute-Ttft-Ms`（首 token 延迟），仅保留正值。缺失或为 0 的 `tok/s` 绝不根据 `latency-ms` 推算（该延迟包含 TTFT，推算值不可靠）。
- **完整遥测写入 `diagnostics`** —— 只要捕获到任意遥测字段（不以是否上报成本为前提），消息即携带 `omniroute-telemetry` 诊断，包含 `model`、`provider`、`tokensIn`、`tokensOut`、`cacheHit`、`responseCost`、`tokensPerSecond`、`ttftMs`、`latencyMs`（未捕获的字段省略），可随时查看任意轮次的路由与速度细节。`usage.cost.total` 仅在存在 `Response-Cost` 时覆盖；token 计数与成本分项绝不被修改。
