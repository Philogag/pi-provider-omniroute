# usage-cost-telemetry 规范

## 目的

将 OmniRoute 路由网关通过响应 body SSE 注释行（`: x-omniroute-*`）下发的成本遥测接入 Pi 的计费统计：会话消息成本反映 `X-OmniRoute-Response-Cost` 报告的真实美元金额（缓存命中按 $0 计费），完整遥测（模型、provider、tokens、缓存命中、延迟）以 `omniroute-telemetry` 类型附加到消息 `diagnostics`。
## 需求
