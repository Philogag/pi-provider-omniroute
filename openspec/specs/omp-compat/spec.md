...
本 capability 保证扩展可在标准 Pi 与 oh-my-pi（omp，捆绑 pi-ai 且缺失部分导出）两种宿主上正常加载与工作。omp 捆绑的 pi-ai 不含 `appendAssistantMessageDiagnostic` 导出，扩展不得依赖该符号；遥测诊断改为直接操作 `message.diagnostics` 数组实现，宿主无关。
## 需求
