# web-fetch-provider-config 规范

## 目的
定义 Web Fetch API（`omniroute_web_fetch` 工具）默认 provider 的可配置能力：通过 `/omniroute-settings` TUI 选择 provider，持久化到 pi 全局 `omniroute.json` 的 `fetch.provider` 字段，并在工具 execute 阶段按"显式入参 > 已配置 > 省略"三态合并。与 search provider 配置（`search.provider`）独立，互不影响。
## 需求
