...
OpenSpec 5 需求 / 14 场景 + 边界场景 G1-G7（`superpower-design.md` §11）已合并入 delta spec（plan Task 0 在 apply 阶段执行）。完整 TDD 蓝图见 `superpower-plan.md`（10 任务 50 步），按 subagent-driven-development 或 executing-plans 实施；每任务独立 commit，commit message 体现设计意图。
